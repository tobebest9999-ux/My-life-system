  ...
  return (
    <div className="dashboard-sketch-layout">
      ...
      <section className="panel sketch-full-row">
        ...
        {sessions.length === 0 ? <p className="empty-text">{T.noRecords}</p> : <DashboardRecentSessions sessions={sessions} />}
      </section>
...
function DashboardRecentSessions({ sessions }: { sessions: Session[] }) {
  const ordered = [...sessions].sort((a, b) => b.startTime.localeCompare(a.startTime));
  return <div className="dashboard-recent-groups">{mainCategoryOptions.map((category) => { const categorySessions = ordered.filter((session) => session.mainCategory === category).slice(0, 3); return <section className="dashboard-recent-group" key={category}><div className="dashboard-recent-group-head"><h3>{mainCategoryLabels[category]}</h3><span>{categorySessions.length} {'\u6761'}</span></div>{categorySessions.length === 0 ? <p className="empty-text">{'\u8fd8\u6ca1\u6709' + mainCategoryLabels[category] + '\u8bb0\u5f55'}</p> : <SessionList sessions={categorySessions} />}</section>; })}</div>;
}
...
function ProjectPlanPage({ projects, tasks, onBack, onSaveTask, onDeleteTask, onOpenProject }: { projects: Project[]; tasks: ProjectTask[]; onBack: () => void; onSaveTask: (task: ProjectTask) => Promise<void>; onDeleteTask: (task: ProjectTask) => Promise<void>; onOpenProject: (id: string) => void }) {
  ...
  const moveDay = (days: number) => { const next = new Date(selectedDate + 'T00:00:00'); next.setDate(next.getDate() + days); setSelectedDate(toDateKey(next)); setSelectedTaskId(''); };
  const create = async () => { if (!projectId || !title.trim() || isPast) return; const timestamp = nowIso(); const task: ProjectTask = { id: createId(), projectId, date: selectedDate, title: title.trim(), status: 'active', note: '', createdAt: timestamp, updatedAt: timestamp }; await onSaveTask(task); setTitle(''); setSelectedTaskId(task.id); };
  const dayTasks = tasks.filter((task) => task.date === selectedDate).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
  const selectedTask = dayTasks.find((task) => task.id === selectedTaskId) ?? null;
  if (selectedTask) return <div className="notion-page project-plan-page"><section className="notion-database-header"><button className="notion-back" onClick={() => setSelectedTaskId('')}>{'\u2190'} {commonUi.back}</button><div className="notion-breadcrumb">{'\u9879\u76ee / \u8ba1\u5212\u8868 / \u4efb\u52a1\u8be6\u60c5'}</div><h2>{'\u4efb\u52a1\u8be6\u60c5'}</h2><p>{'\u67e5\u770b\u8fd9\u4e2a\u4efb\u52a1\u7684\u5173\u8054\u9879\u76ee\u3001\u5b8c\u6210\u72b6\u6001\u548c\u5907\u6ce8\u3002'}</p></section><section className="panel project-plan-panel"><ProjectTaskDetail task={selectedTask} project={projects.find((project) => project.id === selectedTask.projectId)} onSaveTask={onSaveTask} onOpenProject={onOpenProject} /></section></div>;
  return <div className="notion-page project-plan-page"><section className="notion-database-header"><button className="notion-back" onClick={onBack}>{'\u2190'} {commonUi.back}</button><div className="notion-breadcrumb">{'\u9879\u76ee / \u8ba1\u5212\u8868'}</div><h2>{'\u8ba1\u5212\u8868'}</h2><p>{'\u6309\u65e5\u671f\u7ed9\u5177\u4f53\u9879\u76ee\u62c6\u4efb\u52a1\uff0c\u53ef\u4ee5\u63d0\u524d\u5199\u660e\u5929\u6216\u540e\u5929\u7684\u4efb\u52a1\u3002'}</p></section><section className="panel project-plan-panel"><div className="study-plan-date-tools project-plan-date-tools"><button className="ghost-button compact" onClick={() => moveDay(-1)}>{'\u4e0a\u4e00\u5929'}</button><strong>{formatDate(selectedDate)}</strong><button className="ghost-button compact" onClick={() => moveDay(1)}>{'\u4e0b\u4e00\u5929'}</button><label><span>{'\u9009\u62e9\u65e5\u671f'}</span><input type="date" value={selectedDate} onChange={(event) => { setSelectedDate(event.target.value || toDateKey(new Date())); setSelectedTaskId(''); }} /></label></div>{!isPast ? <div className="study-library-toolbar"><select value={projectId} onChange={(event) => setProjectId(event.target.value)}>{projects.map((project) => <option key={project.id} value={project.id}>{projectSubCategoryLabels[project.subCategory as ProjectSubCategory]} / {project.name}</option>)}</select><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={'\u4f8b\u5982\uff1a\u4e3b\u89d2\u79fb\u52a8\u529f\u80fd'} /><button className="primary-button" onClick={create}>{'\u65b0\u589e\u4efb\u52a1'}</button></div> : <p className="muted">{'\u8fc7\u53bb\u7684\u4efb\u52a1\u4e0d\u518d\u65b0\u589e\u6216\u4fee\u6539\uff0c\u4f46\u53ef\u4ee5\u8865\u52fe\u5b8c\u6210\u3002'}</p>}{dayTasks.length === 0 ? <p className="empty-text">{'\u8fd9\u4e00\u5929\u8fd8\u6ca1\u6709\u9879\u76ee\u4efb\u52a1'}</p> : <div className="project-task-list">{dayTasks.map((task) => <ProjectTaskRow key={task.id} task={task} projects={projects} onSaveTask={onSaveTask} onDeleteTask={onDeleteTask} onOpenProject={onOpenProject} onOpenTask={setSelectedTaskId} selected={task.id === selectedTaskId} />)}</div>}</section></div>;
}
...
function ProjectTaskRow({ task, projects, onSaveTask, onDeleteTask, onOpenProject, onOpenTask, selected = false }: { task: ProjectTask; projects: Project[]; onSaveTask: (task: ProjectTask) => Promise<void>; onDeleteTask?: (task: ProjectTask) => Promise<void>; onOpenProject: (id: string) => void; onOpenTask?: (id: string) => void; selected?: boolean }) {
  ...
  return <article className={'project-task-row ' + task.status + (selected ? ' selected' : '')} onClick={() => onOpenTask?.(task.id)} role={onOpenTask ? 'button' : undefined} tabIndex={onOpenTask ? 0 : undefined} onKeyDown={(event) => { if (onOpenTask && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); onOpenTask(task.id); } }}><div className="project-task-main"><strong>{task.title}</strong>{task.note ? <span>{'\u6709\u5907\u6ce8'}</span> : null}</div><button className="ghost-button compact" onClick={(event) => { event.stopPropagation(); onOpenProject(task.projectId); }}>{project ? project.name : '\u67e5\u770b\u9879\u76ee'}</button><time>{task.date}</time>{onDeleteTask ? <button className="danger-button compact" onClick={(event) => { event.stopPropagation(); void onDeleteTask(task); }}>{commonUi.delete}</button> : null}<label className="project-task-check" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={task.status === 'completed'} onChange={complete} /><span>{'\u5b8c\u6210'}</span></label></article>;
}

function ProjectTaskDetail({ task, project, onSaveTask, onOpenProject }: { task: ProjectTask; project?: Project; onSaveTask: (task: ProjectTask) => Promise<void>; onOpenProject: (id: string) => void }) {
  const [note, setNote] = useState(task.note ?? '');
  useEffect(() => setNote(task.note ?? ''), [task.id, task.note]);
  const save = async () => { await onSaveTask({ ...task, note: note.trim() }); };
  return <div className="project-task-detail"><div className="project-task-detail-head"><div><span>{'\u4efb\u52a1\u8be6\u60c5'}</span><h3>{task.title}</h3><p>{project ? projectSubCategoryLabels[project.subCategory as ProjectSubCategory] + ' / ' + project.name : '\u672a\u5173\u8054\u9879\u76ee'} / {formatDate(task.date)}</p></div>{project ? <button className="secondary-button compact" onClick={() => onOpenProject(project.id)}>{'\u67e5\u770b\u9879\u76ee'}</button> : null}</div><label className="stacked-field"><span>{'\u5907\u6ce8'}</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={'\u53ef\u4ee5\u8bb0\u5f55\u8fd9\u4e2a\u4efb\u52a1\u7684\u601d\u8def\u3001\u95ee\u9898\u6216\u540e\u7eed\u8865\u5145'} rows={7} /></label><button className="primary-button compact" onClick={save}>{'\u4fdd\u5b58\u5907\u6ce8'}</button></div>;
}
